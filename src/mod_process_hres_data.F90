...
! Approaches 1 and 2 (move HWSD peat to mineral Group 3)
! ------------------------------------------------------
!         where (oc_top*sf >= cF_lim(4))
!            oc_top = NINT(8./sf)
!         endwhere

! Approach 1,2, and 3 (move HWSD subsurface peat to Group 3)
! -------------------------------------------------       
         where (oc_sub*sf >= cF_lim(4))
            oc_sub = NINT(8./sf)
         endwhere

! Approaches 1 and 3 (OC 1km PEATMAP defines surface OC)
! ------------------------------------------------------
	 ...
	 fac      = soil_class_com(n)
         ...
         if(process_peat) then 
          ...
          ! the maximum peat soil depth is set to the value Michel used to derive parameters (1334.) 
           soildepth(n) = min(soildepth(n),1334.) 

!        Approaches 1 and 3 (reset profile to 253 if surface soil type is 253)
!        --------------------------------------------------------------------- 
           if (fac_surf == 253)  fac      = 253

!        Approach 2 (reset soil types to 253 if peat fraction in tile exceeds pmap_thresh
!        ----------------------------------------------------------------------------------
